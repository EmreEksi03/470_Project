...
class AlgorithmSelector:
    ...
    def prepare_meta_dataset(self, datasets):
        ...
        # ✅ Print distribution of best algorithms
        label_distribution = Counter(meta_y)
        print(f"\nBest algorithm distribution in meta_y:")
        for alg, count in label_distribution.items():
            print(f"  {alg}: {count} times")

        # ✅ Save meta-dataset to CSV
        try:
            df_meta = pd.DataFrame(meta_X, columns=self.meta_features_columns)
            df_meta['best_algorithm'] = meta_y
            df_meta.to_csv("meta_dataset.csv", index=False)
            print("Saved meta-dataset to 'meta_dataset.csv'")
        except Exception as e:
            print(f"Failed to save meta-dataset: {e}")
